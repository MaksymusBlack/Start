...
IMAGES_FORMATS = ["jpeg", "png", "jpg", "svg", "bmp"]
...
def remove_file(file, new_folder_path):
    ...
    try:
        ...
        if os.path.exists(destination_path):                     #Перевіряє, чи є файл з таким ім'ям
                ...
                # counter = 1
                # while os.path.exists(destination_path):              # Якщо є, перейменує файл з додатком copy
...
def check_file_formats_version_two(path):
    ...
    try:
        for i in path.iterdir():
            ...
            if i.is_file() == True:
                ...
                # варіант для папок - функція запускає себе знову
            if i.is_dir() == True:
                check_file_formats_version_two(i)
